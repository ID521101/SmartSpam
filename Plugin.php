<?php
/**
 * 智能评论过滤器，让机器人彻底远离你！<br>
 * <strong style="color:red;">增添违规词文本！在prohibited_words.txt 中添加屏蔽词，一词一行！</strong>
 * 
 * @package SmartSpam
 * @author YoviSun
 * @version 2.8.0
...
class SmartSpam_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function config(Typecho_Widget_Helper_Form $form)
    {
        ...
        $opt_au = new Typecho_Widget_Helper_Form_Element_Radio('opt_au', array("none" => "无动作", "waiting" => "标记为待审核", "spam" => "标记为垃圾", "abandon" => "评论失败"), "abandon",
            _t('屏蔽昵称关键词操作'), "如果评论发布者的昵称含有 prohibited_words.txt (文本为空不触发)中的关键词，将执行该操作");
        $form->addInput($opt_au);

        
        
        
        $au_length_min = new Typecho_Widget_Helper_Form_Element_Text('au_length_min', NULL, '3', '昵称最短字符数', '昵称允许的最短字符数。');
        ...
        $opt_ban = new Typecho_Widget_Helper_Form_Element_Radio('opt_ban', array("none" => "无动作", "waiting" => "标记为待审核", "spam" => "标记为垃圾", "abandon" => "评论失败"), "abandon",
            _t('禁止词汇操作'), "如果评论中包含有 prohibited_words.txt (文本为空不触发)中的关键词汇，将执行该操作");
    ...
    public static function filter($comments, $post, $last)
    {
        ...
        // 只读取一次本地文件
        $prohibitedWordsFile = dirname(__FILE__) . '/prohibited_words.txt';
        $prohibitedWords = file_exists($prohibitedWordsFile) ? file_get_contents($prohibitedWordsFile) : "";

        //游客进行评论进行权限处理
        if($opt == "none" && isset($filter_set->opt_visitor) && $filter_set->opt_visitor != "none" && !$user->hasLogin()){
             $error = "对不起，本站暂时禁止游客进行评论！";
             $opt = $filter_set->opt_visitor;
        }

        //屏蔽评论内容包含文章标题
        if ($opt == "none" && isset($filter_set->opt_title) && $filter_set->opt_title != "none") {
             $db = Typecho_Db::get();
             // 获取评论所在文章
        ...
        //屏蔽IP段处理
        if ($opt == "none" && isset($filter_set->opt_ip) && $filter_set->opt_ip != "none") {
        ...
        // //屏蔽邮箱处理
        // if ($opt == "none" && isset($filter_set->opt_mail) && $filter_set->opt_mail != "none") {
        ...
        //改(旧版在输入屏蔽邮箱后又删除了邮箱就会导致屏蔽所有的邮箱) 屏蔽邮箱处理
        if ($opt == "none" && $filter_set->opt_mail != "none") {
            // 在调用 check_in() 前先检查 words_mail 是否为空
            if (!empty($filter_set->words_mail) && SmartSpam_Plugin::check_in($filter_set->words_mail, $comment['mail'])) {
                $error = "评论发布者的邮箱地址被管理员屏蔽！";
                $opt = $filter_set->opt_mail;
            }
        }
        ...
        //屏蔽网址处理
        if(isset($filter_set->words_url) && !empty($filter_set->words_url)){
            if ($opt == "none" && isset($filter_set->opt_url) && $filter_set->opt_url != "none") {
        ...
        //屏蔽昵称关键词处理
        if ($opt == "none" && isset($filter_set->opt_au) && $filter_set->opt_au != "none" && !empty($prohibitedWords)) {
            if (SmartSpam_Plugin::check_in($prohibitedWords, $comment['author'])) {
        ...
        //日文评论处理
        if ($opt == "none" && isset($filter_set->opt_nojp) && $filter_set->opt_nojp != "none") {
        ...
        //日文用户昵称处理
        if ($opt == "none" && isset($filter_set->opt_nojp_au) && $filter_set->opt_nojp_au != "none") {
        ...
        //昵称长度检测
        if ($opt == "none" && isset($filter_set->opt_au_length) && $filter_set->opt_au_length != "none") {      
            ...
            } else if(SmartSpam_Plugin::strLength($comment['author']) >  $filter_set->au_length_max){        
        ...
        //用户昵称网址判断处理
        if ($opt == "none" && isset($filter_set->opt_nourl_au) && $filter_set->opt_nourl_au != "none") {
        ...
        // //纯中文评论处理
        // if ($opt == "none" && isset($filter_set->opt_nocn) && $filter_set->opt_nocn != "none") {
        ...
        if ($opt == "none" && $filter_set->opt_nocn != "none") {
            $chineseCharCount = preg_match_all("/[\x{4e00}-\x{9fa5}]/u", $comment['text']);
            if ($chineseCharCount < 3) {
                $error = "评论内容请不少于三个中文汉字！";
                $opt = $filter_set->opt_nocn;
            }
        }
        ...
        //字符长度检测
        if ($opt == "none" && isset($filter_set->opt_length) && $filter_set->opt_length != "none") {      
            ...
            } else if(SmartSpam_Plugin::strLength($comment['text']) >  $filter_set->length_max){        
        ...
        //检查禁止词汇
        if ($opt == "none" && isset($filter_set->opt_ban) && $filter_set->opt_ban != "none" && !empty($prohibitedWords)) {
            if (SmartSpam_Plugin::check_in($prohibitedWords, $comment['text'])) {
                $error = "评论内容中包含禁止词汇！";
        ...
        //检查敏感词汇
        if ($opt == "none" && isset($filter_set->opt_chk) && $filter_set->opt_chk != "none") {
